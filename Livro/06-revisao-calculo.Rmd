...
tibble(
  Função = c("$f(x)=c$ (constante)", "$f(x)=x^n$", "$f(x)=e^x$", "$f(x)=\\ln(x)$",
             "$f(x)=c\\cdot g(x)$", "$f(x)=g(x)+h(x)$", "$f(x)=g(x)\\,h(x)$", "$f(x)=g(h(x))$ (cadeia)"),
  Derivada = c("$f'(x)=0$", "$f'(x)=nx^{n-1}$", "$f'(x)=e^x$", "$f'(x)=1/x$",
               "$f'(x)=c\\cdot g'(x)$", "$f'(x)=g'(x)+h'(x)$", "$f'(x)=g'(x)h(x)+g(x)h'(x)$", "$f'(x)=g'(h(x))\\,h'(x)$"),
  Exemplo = c("$f(x)=3 \\Rightarrow f'(x)=0$", "$f(x)=2x^5 \\Rightarrow f'(x)=10x^4$", "$f(x)=e^{2x} \\Rightarrow f'(x)=2e^{2x}$",
              "$f(x)=\\ln(2x) \\Rightarrow f'(x)=1/x$", "$f(x)=5e^x \\Rightarrow f'(x)=5e^x$",
              "$f(x)=e^{2x}+x^2 \\Rightarrow f'(x)=2e^{2x}+2x$", "$f(x)=x^2\\ln x \\Rightarrow f'(x)=2x\\ln x + x$",
              "$f(x)=e^{-x^2} \\Rightarrow f'(x)=-2x\\,e^{-x^2}$")
...
de chegar lá: por que faz sentido que a <em>taxa de variação</em> da probabilidade acumulada em um
...
tibble(
  Função = c("$f(x)=c$", "$f(x)=x^n$ ($n\\ne -1$)", "$f(x)=1/x$", "$f(x)=e^{\\lambda x}$", "$f(x)=e^{-x}$"),
  Primitiva = c("$F(x)=cx$", "$F(x)=\\dfrac{x^{n+1}}{n+1}$", "$F(x)=\\ln(x)$, $x>0$", "$F(x)=\\dfrac{1}{\\lambda}e^{\\lambda x}$", "$F(x)=-e^{-x}$")
...
- $\int_1^{10} dx = [x]_1^{10} = 10 - 1 = 9$.
- $\int_0^1 x^n\,dx = \left[\frac{x^{n+1}}{n+1}\right]_0^1 = \frac{1}{n+1}$.
- $\int_0^{\infty} e^{-x}\,dx = \lim_{t \to \infty}\left[-e^{-x}\right]_0^t = 0 - (-1) = 1$.

**Exercício 1.** Calcule as derivadas: **a)** $x^5$; **b)** $2 + x^3$; **c)** $5e^x$; **d)** $\ln(x)$; **e)** $2x^2 + 3x^3$.

::: {.caixa-resposta}
**a)** $5x^4$; **b)** $3x^2$; **c)** $5e^x$; **d)** $1/x$; **e)** $4x + 9x^2$.
:::

**Exercício 2.** Calcule **a)** $\int_1^2 2x^3\,dx$; **b)** $\int_5^8 \frac{1}{x}\,dx$; **c)** $\int_{-1}^{2} (2 + e^{-x})\,dx$; **d)** $\int_0^1 3x^2\,dx$.

::: {.caixa-resposta}
**a)** $\left[\frac{x^4}{2}\right]_1^2 = 8 - \frac{1}{2} = \frac{15}{2}$. **b)** $\ln 8 - \ln 5 = \ln(1{,}6) \approx 0{,}47$.
**c)** $\left[2x - e^{-x}\right]_{-1}^{2} = (4 - e^{-2}) - (-2 - e) = 6 + e - e^{-2} \approx 8{,}58$.
**d)** $[x^3]_0^1 = 1$: não por coincidência, $f(x) = 3x^2$ em $[0, 1]$ é uma densidade de probabilidade.
:::

```{r exercicios-calculo}
D(quote(2 * x^2 + 3 * x^3), "x")
c(a = integrate(function(x) 2 * x^3, 1, 2)$value,
  b = integrate(function(x) 1 / x, 5, 8)$value,
  c = integrate(function(x) 2 + exp(-x), -1, 2)$value,
  d = integrate(function(x) 3 * x^2, 0, 1)$value)
```
