...
sinistros |>
  kbl(col.names = c("x (nº de sinistros)", "p(x)")) |>
  kable_styling(full_width = FALSE)
```

```{r grafico-fmp, fig.height=3.5, echo=FALSE}
ggplot(sinistros, aes(x = factor(x), y = p)) +
  geom_col(fill = "#0d3b54") +
  labs(x = "Número de sinistros", y = "p(x)") + theme_minimal(base_size = 13)
```
...
`r round(mu,2)`\) sinistros por apólice ao ano é exatamente o número que a seguradora usa para
...
<p><strong>1.</strong> \(E(X+Y)=E(X)+E(Y)\) vale <em>sempre</em>, mesmo se \(X\) e \(Y\) forem
...
<p><strong>3.</strong> Se a mesma seguradora quisesse calcular a <em>variância</em> do número total
...
```{r grafico-fda, fig.height=3.5, echo=FALSE}
ggplot(sinistros, aes(x = x, y = F)) +
  geom_step(color = "#0d3b54", linewidth = 1) +
  geom_point(color = "#c9971e", size = 2) +
  labs(x = "x", y = "F(x) = P(X ≤ x)", title = "FDA, número de sinistros") +
...
```{r fig-fda-dado, echo=FALSE, out.width="65%", fig.cap="FDA do resultado do lançamento de um dado honesto: seis saltos de tamanho 1/6, uma escada perfeitamente regular."}
...
```{r fig-pesquisa-celular, echo=FALSE, out.width="70%", fig.cap="86% dos brasileiros apoiam restrição de celular nas escolas, segundo pesquisa Datafolha/Agência Brasil."}
...
Antes de qualquer chamada ao R, vale expandir a fórmula à mão pelo menos uma vez, para deixar claro
de onde vêm os números que `dbinom()`/`pbinom()` calculam por trás dos panos.

```{=html}
<div class="caixa-aplicacao"><strong>Exemplo resolvido: garantia por peça defeituosa (cálculo
manual antes do R)</strong>: uma fábrica produz peças com \(1\%\) de probabilidade de defeito,
independentemente umas das outras. Cada pacote tem 10 peças, e a fábrica devolve o dinheiro se
<strong>mais de uma</strong> peça do pacote for defeituosa. Qual a probabilidade de a fábrica
precisar devolver o dinheiro por um pacote sorteado ao acaso?</div>
```

Sejam $p=0{,}01$ e $X\sim\text{Bin}(10; 0{,}01)$ o número de peças defeituosas no pacote. Queremos
$P(X>1) = 1-P(X\le 1) = 1-P(X=0)-P(X=1)$. Expandindo a fórmula da Binomial para cada termo:

$$
P(X=0) = \binom{10}{0}(0{,}01)^0(0{,}99)^{10} = 1\cdot 1\cdot(0{,}99)^{10} \approx 0{,}9044,
$$

$$
P(X=1) = \binom{10}{1}(0{,}01)^1(0{,}99)^{9} = 10\cdot 0{,}01\cdot(0{,}99)^{9}
\approx 10\times 0{,}01\times 0{,}9135 \approx 0{,}0914.
$$

Logo, $P(X>1) \approx 1-0{,}9044-0{,}0914 = 0{,}0042$: cerca de 0,42% dos pacotes exigirão
devolução. Conferindo no R (que usa mais casas decimais e por isso é ligeiramente mais preciso que
o cálculo manual arredondado acima):

```{r binomial-garantia}
1 - pbinom(1, size = 10, prob = 0.01)
```

```{=html}
<div class="caixa-aplicacao"><strong>Exemplo resolvido: de quanto preciso amostrar?</strong>: ainda
com \(p=0{,}01\) de defeito, qual o menor tamanho de amostra \(n\) tal que a probabilidade de encontrar
<strong>pelo menos um</strong> item defeituoso seja de no mínimo 95%?</div>
```

Com $X\sim\text{Bin}(n; 0{,}01)$, temos $P(X\ge 1) = 1-P(X=0) = 1-(0{,}99)^n$. Queremos

$$
1-(0{,}99)^n \ge 0{,}95 \iff (0{,}99)^n \le 0{,}05.
$$

Aplicando logaritmo em ambos os lados (a desigualdade se inverte porque $\log(0{,}99)<0$):

$$
n \ge \frac{\log(0{,}05)}{\log(0{,}99)} \approx \frac{-2{,}996}{-0{,}01005} \approx 298{,}07
\ \Longrightarrow\ n=299.
$$

```{r binomial-n-minimo, fig.height=3.4}
n_vals <- 1:400
prob_pelo_menos_um <- 1 - dbinom(0, size = n_vals, prob = 0.01)
which(prob_pelo_menos_um >= 0.95)[1]   # confirma n = 299
```

```{r grafico-n-minimo, echo=FALSE, fig.height=3.4}
ggplot(tibble(n = n_vals, prob = prob_pelo_menos_um), aes(n, prob)) +
  geom_line(color = "#0d3b54") +
  geom_hline(yintercept = 0.95, color = "#c9971e", linetype = "dashed") +
  labs(x = "Tamanho da amostra (n)", y = "P(pelo menos 1 defeituoso)") +
  theme_minimal(base_size = 13)
```

```{r binomial-r, fig.height=3.6}
n <- 20; p <- 0.08
x <- 0:n
...
```{r grafico-binomial, echo=FALSE, fig.height=3.6}
ggplot(tibble(x, fmp), aes(x, fmp)) +
  geom_col(fill = "#0d3b54") +
  labs(x = "Nº de inadimplentes (de 20)", y = "P(X = x)",
       title = "Binomial(n=20, p=0,08)") + theme_minimal(base_size = 13)
```
...
```{=html}
<div class="caixa-aplicacao"><strong>Exemplo resolvido: calculando \(P(X=x)\) à mão</strong>: uma
central de atendimento recebe, em média, \(\lambda=3{,}2\) chamadas por minuto. Qual a probabilidade
de receber exatamente 4 chamadas em um minuto?</div>
```

$$
P(X=4) = \frac{e^{-3{,}2}\,(3{,}2)^4}{4!} = \frac{0{,}0408\times 104{,}8576}{24} \approx 0{,}1781.
$$

Conferindo no R, o cálculo manual e a função pronta devem coincidir (a menos de arredondamento):

```{r poisson-manual}
exp(-3.2) * 3.2^4 / factorial(4)   # cálculo manual, passo a passo
dpois(4, lambda = 3.2)             # função pronta do R
```
...
3.1) e de modelos de contagem em econometria aplicada
(Wooldridge (<a href="#ref-wooldridge2016introducao">2016</a>)).</div>
```
...
```{r fig-noticia-acidentes, echo=FALSE, out.width="65%", fig.cap="PRF registra 165 mortes em rodovias federais na semana do Natal de 2024, ante 90 mortes no mesmo período de 2023."}
...
```{r fig-noticia-gols, echo=FALSE, out.width="65%", fig.cap="PSG 5 x 4 Bayern: recorde de gols em uma partida de ida de semifinal da Champions League (abril de 2026)."}
...
```{=html}
<div class="caixa-aplicacao"><strong>Exemplo resolvido: Binomial vs. aproximação Poisson</strong>:
um produto tem \(1\%\) de probabilidade de defeito. Numa amostra de 200 unidades, qual a
probabilidade de no máximo 2 serem defeituosas? Calculando exatamente pela Binomial e pela
aproximação Poisson com \(\lambda=np=200\times 0{,}01=2\):</div>
```

```{r binom-vs-poisson-livro}
pbinom(2, size = 200, prob = 0.01)   # exato, Bin(200; 0,01)
ppois(2, lambda = 2)                 # aproximação, Poisson(2)
```

As duas probabilidades praticamente coincidem (diferença apenas na quarta casa decimal),
confirmando que, com $n$ grande e $p$ pequeno, a Poisson aproxima muito bem a Binomial, e com um
cálculo bem mais simples (sem fatoriais de 200).

```{=html}
<div class="caixa-aplicacao"><strong>Exemplo resolvido: estimando \(\lambda\) a partir de
dados</strong>: o número de mortes causadas por uma doença, nos últimos sete anos, foi
\(2, 3, 3, 2, 2, 1, 1\). Supondo um modelo Poisson adequado, qual a probabilidade de 4 ou mais mortes
no ano seguinte?</div>
```

O parâmetro $\lambda$ de uma Poisson é também a sua média, então a estimativa natural é a média
amostral: $\hat\lambda = \bar x = (2+3+3+2+2+1+1)/7 = 2$. Sob o modelo $\text{Poisson}(2)$:

```{r poisson-estimacao}
mortes <- c(2, 3, 3, 2, 2, 1, 1)
lambda_hat <- mean(mortes)
lambda_hat
1 - ppois(3, lambda = lambda_hat)   # P(X >= 4) = 1 - P(X <= 3)
```

Esse é um primeiro exemplo, ainda informal, de uma ideia que disciplinas de inferência formalizam
mais adiante: **estimar um parâmetro a partir de dados observados** e usá-lo para prever o
comportamento futuro da variável aleatória.

```{r poisson-r, fig.height=3.6}
...
```{r grafico-poisson, echo=FALSE, fig.height=3.6}
ggplot(tibble(x, fmp_pois), aes(x, fmp_pois)) +
  geom_col(fill = "#c9971e") +
  labs(x = "Nº de chamadas por minuto", y = "P(X = x)",
       title = paste0("Poisson(λ=", lambda, ")")) + theme_minimal(base_size = 13)
```
...
Até aqui, cada distribuição foi tratada apenas analiticamente: uma fórmula para $p(x)$, calculada
via `dbinom()`/`dpois()`. Há uma segunda forma, complementar, de entender essas distribuições:
**simular** milhares de repetições do experimento aleatório e comparar as estatísticas simuladas
(média, variância amostral) com os valores teóricos $E(X)$ e $\mathrm{Var}(X)$. Essa é a mesma
lógica por trás de qualquer simulação de Monte Carlo, mencionada informalmente no Capítulo
\@ref(variaveis-aleatorias-continuas), e que colocamos em prática aqui pela primeira vez.

A função mais básica para isso é `sample()`, que sorteia valores de um conjunto com probabilidades
dadas. Uma Bernoulli($p$) nada mais é que `sample(c(0,1), n, prob = c(1-p, p), replace = TRUE)`:

```{r simula-bernoulli}
set.seed(42)
p <- 0.3
x <- sample(c(0, 1), size = 1000, prob = c(1 - p, p), replace = TRUE)
c(media_simulada = mean(x), media_teorica = p)
c(var_simulada = var(x), var_teorica = p * (1 - p))
```

Para Binomial e Poisson, o R já tem geradores dedicados, `rbinom()` e `rpois()`, que fazem
internamente algo equivalente a somar $n$ Bernoullis (Binomial) ou sortear contagens raras
(Poisson):

```{r simula-binom-pois}
y <- rbinom(1000, size = 20, prob = 0.08)   # carteira de microcrédito, seção anterior
c(media_simulada = mean(y), media_teorica = 20 * 0.08)
c(var_simulada = var(y), var_teorica = 20 * 0.08 * (1 - 0.08))

z <- rpois(1000, lambda = 3.2)                 # central de atendimento
c(media_simulada = mean(z), media_teorica = 3.2)
c(var_simulada = var(z), var_teorica = 3.2)
```

Em todos os três casos, média e variância simuladas ficam próximas dos valores teóricos, mas não
idênticas: a diferença encolhe à medida que o número de repetições simuladas cresce, a mesma
lógica de estabilização de frequências relativas discutida no Capítulo 2. Essa é a ideia central de
qualquer simulação de Monte Carlo: usar sorteios aleatórios repetidos como substituto
computacional para um cálculo teórico, útil sobretudo quando o cálculo exato é difícil ou
impossível de obter analiticamente.

```{=html}
<div class="caixa-r"><strong>Uso do R</strong>: o padrão <code>r</code> + nome da distribuição
(<code>rbinom</code>, <code>rpois</code> e, de forma mais genérica, <code>sample()</code>) gera
valores aleatórios; combinado com <code>mean()</code> e <code>var()</code>, permite checar
empiricamente qualquer fórmula teórica de \(E(X)\) e \(\mathrm{Var}(X)\) vista neste capítulo.</div>
```
