...
**Razão e taxa de variação.** Para categorias com $f_1$ e $f_2$ casos, ou para o valor de uma
variável em dois instantes $f_1$ (tempo 1) e $f_2$ (tempo 2):
$$
\text{razão}_{1:2} = \frac{f_1}{f_2} \qquad
\text{taxa de variação} = 100\times\frac{f_2-f_1}{f_1}\,\%
$$

**Tendência central.**
$$
\bar x = \frac1n\sum_{i=1}^n x_i = \sum_{i=1}^k f_i x_i\ \text{(tabela de frequências, } f_i=n_i/n)
$$
$$
...
**Percentil de ordem $P$** ($0<P<100$), dados ordenados $x_{(1)}\le\cdots\le x_{(n)}$, $k=Pn/100$:
se $k$ inteiro, percentil $=\dfrac{x_{(k)}+x_{(k+1)}}{2}$; se $k$ não inteiro, arredonde $k$ para
cima e percentil $=x_{(k)}$.

**Dispersão.**
$$
AT = x_{\max}-x_{\min} \qquad IQR = Q_3-Q_1
$$
$$
\mathrm{Var}(X) = \frac1n\sum_{i=1}^n (x_i-\bar x)^2 \qquad dp(X)=\sqrt{\mathrm{Var}(X)} \qquad CV(X) = \frac{dp(X)}{\bar x}
$$
$$
\text{outlier (boxplot): } x < Q_1-1{,}5\,IQR \ \text{ ou } \ x>Q_3+1{,}5\,IQR
$$

**Curtose, três regimes:** leptocúrtica ($>3$, caudas pesadas) $\quad$ mesocúrtica
($\approx3$, referência Normal) $\quad$ platicúrtica ($<3$, caudas leves).

**Tabela de contingência de referência** [@yule1912association]**:**

```{r tabela-yule-formulario, echo=FALSE}
tibble::tribble(
  ~Grupo, ~Recuperados, ~Mortes, ~Total,
  "Vacinados", "197", "2", "199",
  "Não-vacinados", "139", "19", "158",
  "Total", "336", "21", "357"
) |> knitr::kable(caption = "Yule (1912), vacinação × recuperação (n = 357)")
```
...
) |> knitr::kable(escape = FALSE, caption = "Modelos discretos")
```
...
) |> knitr::kable(escape = FALSE, caption = "Modelos contínuos")
```

**Regra empírica (Normal):** $P(\mu\pm\sigma)\approx68{,}3\%$;
$P(\mu\pm2\sigma)\approx95{,}4\%$; $P(\mu\pm3\sigma)\approx99{,}7\%$.

## Cálculo (resumo do Apêndice B) {#apêndice-b-cálculo-resumo}
...
```{r tabela-normal, echo=FALSE}
z <- c(0.5, 0.674, 1.0, 1.282, 1.645, 1.96, 2.0, 2.326, 2.576, 3.0)
tibble::tibble(
  z = z,
  `P(Z ≤ z)` = round(pnorm(z), 4),
  `P(Z > z)` = round(1 - pnorm(z), 4)
) |> knitr::kable(caption = "Valores de referência da FDA da Normal padrão",
                  digits = c(3, 4, 4), format.args = list(decimal.mark = ","))
```
...
Obras e fontes citadas ao longo do livro.
