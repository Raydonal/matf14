...
<p><strong>1.</strong> Se \(P(X=x)=0\) para todo \(x\), como é possível que \(X\) assuma <em>algum</em>
...
é o mesmo tipo de sutileza por trás da integral imprópria do Apêndice B.)</p>
...
$P(a\le X\le b)$ é a **área** sob essa curva entre $a$ e $b$. Essa área é, por definição, o que uma
integral calcula, o limite de uma soma de retângulos cada vez mais finos aproximando a região sob a
curva; quem quiser revisitar essa construção passo a passo (a soma de Riemann) encontra a
demonstração completa no Apêndice \@ref(revisao-de-calculo).

```{r fig-fdp-exemplo-mao, echo=FALSE, out.width="60%", fig.cap="Densidade $f(x)$ genérica: a área sob a curva entre $a$ e $b$ é $P(a<X\\le b)$; a área total sob toda a curva vale 1."}
...
```{r fig-chuva-salvador, echo=FALSE, out.width="70%", fig.cap="Chuva acumulada em Salvador ultrapassa 76% do esperado para novembro em menos de 24 horas (G1, 12/11/2025)."}
...
```{r fdp-exponencial, fig.height=3.5}
...
Antes de deixar o R fazer a conta pela primeira vez, vale ver como ela sai "à mão". Para a fila de
atendimento, $f(x)=0{,}2\,e^{-0{,}2x}$, $x\ge0$, a esperança é

$$
E(X) = \int_0^\infty x\cdot 0{,}2\,e^{-0{,}2x}\,dx.
$$

Essa é uma integral por partes clássica (Apêndice \@ref(revisao-de-calculo)), mas o resultado geral
vale a pena memorizar: para qualquer Exponencial com taxa $\lambda$, $f(x)=\lambda e^{-\lambda x}$,

$$
E(X) = \int_0^\infty x\cdot\lambda e^{-\lambda x}\,dx = \frac{1}{\lambda}.
$$

Com $\lambda=0{,}2$, $E(X) = 1/0{,}2 = 5$. Só agora confirmamos numericamente:

```{r esperanca-exponencial}
# E(X) da fila: integral de x*f(x) de 0 a infinito (à mão, deu 1/lambda = 1/0,2 = 5)
integrand <- function(x) x * 0.2*exp(-0.2*x)
integrate(integrand, lower = 0, upper = Inf)
```

$E(X)=5$ minutos, o tempo médio de espera na fila, exatamente o valor previsto pela fórmula
$1/\lambda$.

```{r fig-peso-esperado, echo=FALSE, out.width="80%", fig.cap="Companhia aérea reforma aviões após o peso real dos passageiros de primeira classe superar o peso esperado usado no projeto das cabines."}
...
relação entre $f$ e $F$ (Teorema Fundamental do Cálculo, Apêndice B):

$$
F'(x) = f(x), \qquad P(a\le X\le b) = F(b) - F(a).
$$

```{r fda-exponencial, fig.height=3.5}
...
```{r fig-fdp-triangular, echo=FALSE, out.width="50%", fig.cap="Densidade triangular em [0,2], com pico em x=1: outro exemplo de densidade válida, sem distribuição nomeada correspondente no R."}
knitr::include_graphics("images/fdp_triangular.jpg")
```
...
```{r fig-hist-unif, echo=FALSE, out.width="65%", fig.cap="Histograma de 1.000 valores simulados de Unif(0,1): a densidade constante (vermelho) e a média teórica 0,5 (azul) já são bem aproximadas por uma amostra desse tamanho."}
...
A padronização por si só não calcula nada, ela só troca a pergunta original (sobre $X$) por uma
pergunta equivalente sobre $Z$, que pode ser respondida numa tabela ou numa função como
`pnorm()`. A relação-chave é

$$
P(X\le a) = P\!\left(Z\le \frac{a-\mu}{\sigma}\right) = \Phi\!\left(\frac{a-\mu}{\sigma}\right),
$$

em que $\Phi$ é a FDA da Normal padrão. Vale a pena fazer essa conta à mão pelo menos uma vez antes
de confiar apenas no R.

```{=html}
<div class="caixa-aplicacao"><strong>Exemplo resolvido: padronização passo a passo</strong>, a
renda mensal (\(X\), em R$) das famílias de uma certa cidade segue aproximadamente
\(X\sim N(2.000,\ 1.000^2)\). Qual é a probabilidade de uma família sorteada ao acaso ganhar menos de
R$ 1.830?</div>
```

**Passo 1 (padronizar).** Subtrai-se a média e divide-se pelo desvio padrão:

$$
z = \frac{1.830-2.000}{1.000} = \frac{-170}{1.000} = -0{,}17.
$$

**Passo 2 (reescrever o evento em termos de $Z$).**

$$
P(X<1.830) = P\!\left(Z < \frac{1.830-2.000}{1.000}\right) = P(Z<-0{,}17) = \Phi(-0{,}17).
$$

**Passo 3 (consultar a tabela ou calcular $\Phi$).** Na tabela da Normal padrão, a linha $-0{,}1$
com a coluna $0{,}07$ fornece $\Phi(-0{,}17)\approx 0{,}4325$: cerca de **43,25%** das famílias
ganham menos de R\$ 1.830. Só agora, com o valor já obtido à mão, confirmamos com `pnorm()`:

```{r padronizacao-mao}
mu_renda <- 2000; sigma_renda <- 1000

z <- (1830 - mu_renda) / sigma_renda
z                       # valor padronizado calculado à mão

pnorm(z)                # Phi(z) pela Normal padrão, N(0,1)
pnorm(1830, mu_renda, sigma_renda)  # mesma probabilidade, direto na escala de X
```

As três formas (tabela, `pnorm(z)` e `pnorm(1830, mu, sigma)`) devem coincidir, e coincidem: o
`pnorm()` com os parâmetros originais já faz a padronização internamente, por isso adiantar o
cálculo de $z$ à mão é redundante para o R, mas essencial para entender *o que* o R está calculando
e para resolver questões de prova sem computador.

```{r fig-black-scholes, echo=FALSE, out.width="70%", fig.cap="'Fórmula de Midas': a equação de Black-Scholes, criada para precificar opções financeiras, apoiada na suposição de que retornos seguem uma distribuição Normal (ou log-Normal)."}
...
histórico sugere retorno mensal aproximadamente \(X\sim N(1{,}2\%,\ (4{,}5\%)^2)\). Qual é a
probabilidade do fundo ter retorno negativo em um mês qualquer?</div>
```

Repetindo os passos do exemplo anterior, agora com $\mu=1{,}2$ e $\sigma=4{,}5$. Padronizando o
valor de interesse ($a=0$):

$$
z = \frac{0-1{,}2}{4{,}5} = \frac{-1{,}2}{4{,}5} \approx -0{,}27,
$$

$$
P(X<0) = P\!\left(Z<\frac{0-1{,}2}{4{,}5}\right) = P(Z<-0{,}27) = \Phi(-0{,}27) \approx 0{,}3936
$$

pela tabela (linha $-0{,}2$, coluna $0{,}07$). Para a segunda pergunta, "qual a probabilidade do
retorno ficar entre 0% e 5% no mês?", padronizamos os dois extremos do intervalo:

$$
z_1 = \frac{0-1{,}2}{4{,}5}\approx -0{,}27, \qquad z_2 = \frac{5-1{,}2}{4{,}5}\approx 0{,}84,
$$

$$
P(0<X<5) = \Phi(z_2)-\Phi(z_1) = \Phi(0{,}84)-\Phi(-0{,}27) \approx 0{,}7995-0{,}3936 = 0{,}4059.
$$

```{r normal-r, fig.height=3.6}
mu <- 1.2; sigma <- 4.5

# padronização à mão, como acima, antes de qualquer chamada a pnorm()
z0 <- (0 - mu) / sigma
z5 <- (5 - mu) / sigma
c(z0 = z0, z5 = z5)

# Phi(z) pela Normal padrão: deve reproduzir os valores de tabela acima
pnorm(z0)
pnorm(z5) - pnorm(z0)

# confirmação: pnorm() com os parâmetros originais já padroniza por dentro
pnorm(0, mean = mu, sd = sigma)
pnorm(5, mu, sigma) - pnorm(0, mu, sigma)
```

Note que `pnorm(z0)` usa $z_0=-0{,}2\overline{6}$ com todas as casas decimais, enquanto a consulta
à tabela acima arredondou $z$ para $-0{,}27$ (tabelas só têm duas casas). Por isso os valores
"batem" apenas de forma aproximada ($0{,}3936$ na tabela vs. $\approx 0{,}3949$ no R): a diferença
é só arredondamento, não um erro de método, e é exatamente o tipo de discrepância que se espera ao
comparar um cálculo manual com uma tabela e um cálculo numérico exato.

```{r grafico-normal, echo=FALSE, fig.height=3.8}
df <- tibble(x = seq(mu - 4*sigma, mu + 4*sigma, length.out = 300),
             y = dnorm(x, mu, sigma))
df_neg <- df |> filter(x <= 0)

ggplot(df, aes(x,y)) + geom_line(color = "#0d3b54") +
  geom_area(data = df_neg, fill = "#c9971e", alpha = .6) +
  labs(x = "retorno mensal (%)", y = "densidade",
       title = paste0("P(retorno < 0) = área sombreada ≈ ",
                       round(pnorm(0, mu, sigma), 3))) +
  theme_minimal(base_size = 13)
```
...
(curtose \(>3\)), de modo que a probabilidade Normal tende a <em>subestimar</em> a frequência real de
