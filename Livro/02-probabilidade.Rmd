...
<em>ou</em> desvalorização cambial" no próximo trimestre, os dois eventos não são mutuamente
...
malha_fina |>
  kbl(col.names = c("Faixa de renda", "Caiu na malha fina", "Não caiu", "Total")) |>
...
<em>pelo menos um</em> desses 2 é
...
```{=html}
<div class="caixa-aplicacao"><strong>Aplicação: voltando à malha fina</strong>, a Seção 2.3
prometeu uma pergunta mais interessante: a chance de cair na malha
...
malha_fina |>
  select(Faixa_de_renda, Caiu_na_malha_fina, Total, P_malha_dado_faixa) |>
  kbl(digits = 4,
      col.names = c("Faixa", "Caiu na malha", "Total", "P(malha | faixa)")) |>
  kable_styling(full_width = FALSE)
```
...
conhece a taxa de inadimplência histórica <em>dentro</em> de cada setor, \(P(A\mid B_i)\). O teorema da
probabilidade total é exatamente a fórmula que a seguradora usa para obter a taxa de inadimplência
<em>global</em> esperada da carteira, ponderando cada taxa setorial pela participação \(P(B_i)\)
daquele setor na carteira total.</div>
```

Antes do exemplo com dados reais, vale fixar o mecanismo em um caso pequeno, com números exatos.
Um lote tem 20 peças defeituosas e 80 peças em perfeito estado. Duas peças são retiradas ao acaso,
sem reposição. Qual é a probabilidade de a **segunda** peça retirada ser defeituosa?

Seja $B=$ "a primeira peça retirada é defeituosa" e $A=$ "a segunda peça retirada é defeituosa".
Como a primeira peça, ou é defeituosa, ou não é, $B$ e $B^c$ formam uma partição do espaço
amostral, com $P(B)=20/100=0{,}2$ e $P(B^c)=80/100=0{,}8$. Se a primeira peça saiu defeituosa,
restam 19 defeituosas em 99 peças; se não saiu, restam 20 defeituosas em 99 peças. Logo,

$$
\begin{aligned}
P(A) &= P(A\mid B)\,P(B) + P(A\mid B^c)\,P(B^c) \\
&= \frac{19}{99}\times\frac{20}{100} + \frac{20}{99}\times\frac{80}{100} \\
&= \frac{380}{9900} + \frac{1600}{9900} = \frac{1980}{9900} = 0{,}2.
\end{aligned}
$$
...
P_A_dado_B  <- 19/99
P_A_dado_Bc <- 20/99
P_A <- P_A_dado_B * P_B + P_A_dado_Bc * P_Bc
P_A
```

Note que $P(A)=0{,}2$ é exatamente igual a $P(B)$, a probabilidade da **primeira** peça ser
defeituosa. Sem observar o resultado da primeira retirada, a segunda peça carrega, a priori, a
mesma chance marginal de ser defeituosa que a primeira, mesmo a extração sendo sem reposição, um
resultado que surpreende quem espera que "sem reposição" sempre distinga a primeira e a segunda
posições.
...
da carteira (a participação \(P(B_i)\) de cada setor) mudasse, mesmo que <em>nenhuma</em> taxa setorial
...
Antes do exemplo mais elaborado desta seção, um caso com números mais simples fixa o mecanismo.
Uma corretora avalia que a probabilidade de a bolsa cair amanhã é $P(B)=0{,}10$ (a priori). Surge a
notícia de que o dólar teve alta hoje, evento $A$. O histórico mostra que, nos dias em que a bolsa
caiu no dia seguinte, o dólar tinha subido no dia anterior em $20\%$ dos casos
($P(A\mid B)=0{,}20$); já nos dias em que a bolsa subiu, o dólar tinha subido no dia anterior em
apenas $5\%$ dos casos ($P(A\mid B^c)=0{,}05$). Qual é a probabilidade de a bolsa cair amanhã, agora
que se sabe que o dólar subiu hoje?

Como $B$ e $B^c$ formam uma partição de $\Omega$, com $P(B^c)=1-0{,}10=0{,}90$, o Teorema de Bayes
dá
...
Aplicando o Teorema de Bayes, com o denominador expandido pelo teorema da probabilidade total
($\text{fraude}$ e $\text{legítima}$ formam uma partição), com os números plugados diretamente na
fórmula:

$$
\begin{aligned}
P(\text{fraude}\mid A) &= \frac{P(A\mid \text{fraude})\,P(\text{fraude})}
{P(A\mid \text{fraude})\,P(\text{fraude}) + P(A\mid \text{legítima})\,P(\text{legítima})} \\
&= \frac{0{,}95\times 0{,}002}{0{,}95\times 0{,}002 + 0{,}03\times 0{,}998} \\
...
suspeita ser <em>de fato</em> uma fraude é de apenas
...
2.4) e o exemplo do antifraude têm uma raiz comum: confundir
