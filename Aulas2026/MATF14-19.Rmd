...
\\(F(2)\\) não vem de uma função mágica, é a soma na mão das probabilidades individuais até \\(x=2\\):

$$
F(2) = p(0)+p(1)+p(2) = 0{,}60+0{,}25+0{,}10 = 0{,}95
$$
...
```{r fig-fda-dado, echo=FALSE, out.width="40%"}
...
\\(p(x\_i)\\), porque ali a distribuição não é uniforme. Mesma definição de \\(F\\), formato da escada
...
\\(p(x\_i)\\) para cada \\(x\_i\\)? Escreva a fórmula geral usando a diferença de dois valores de \\(F\\), e
