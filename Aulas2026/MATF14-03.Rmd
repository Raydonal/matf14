...
```{r fig-eixo-zero, echo=FALSE, fig.height=3.6}
...
## Provocação 1: a mesma base, eixo cortado

Exatamente os **mesmos três números**, eixo começando perto do menor valor:

```{r fig-eixo-cortado, echo=FALSE, fig.height=3.6}
ggplot(fat_regiao, aes(x = fct_reorder(regiao, faturamento_medio), y = faturamento_medio)) +
...
Para \\(f\_i\\) casos na categoria \\(i\\), de um total \\(n=\sum\_i f\_i\\):

$$
\text{Proporção}\_i = \frac{f\_i}{n}, \qquad \text{Porcentagem}\_i = 100\times\frac{f\_i}{n}\,\%
$$
...
**Razão**: para \\(f\_1\\) casos numa categoria e \\(f\_2\\) em outra, razão\\(\_{1:2} = f\_1/f\_2\\)
(lida como "para cada \\(n\\) da categoria 2, há \\(n\times f\_1/f\_2\\) da categoria 1").

--

**Taxa de variação**: para o valor de uma variável em dois instantes, \\(f\_1\\) e \\(f\_2\\),
...
É **literalmente** a fórmula do IPCA mês a mês: \\(f\_1\\) é o índice do mês anterior, \\(f\_2\\) o
atual.

```{r taxa-variacao-empresa1}
e1 <- empresas |> filter(Empresa == 1)
100 * (e1$faturamento99 - e1$faturamento98) / e1$faturamento98   # taxa de variação, empresa 1
```
...
## Aplicação histórica: a tabela de Yule (1912)
...
Yule, G. U. (1912), estudo sobre vacinação contra a varíola, uma das tabelas de dupla entrada mais
citadas da história da Estatística.

--

Proporção de mortes: vacinados \\(2/199\approx1{,}0\%\\); não-vacinados \\(19/158\approx12{,}0\%\\).
Quatro células já carregam a evidência quantitativa central do argumento a favor da vacinação.
...
## Uso correto de linha: três variáveis ao mesmo tempo

Sexo, raça e o ano (esse sim uma sequência temporal):

```{r fig-forca-trabalho, echo=FALSE, out.width="58%"}
...
```{r fig-gender-pay, echo=FALSE, out.width="52%"}
knitr::include_graphics("images/gender_pay_gap.png")
```

Eixo x = classes de diferença salarial; eixo y = número de empresas naquela classe. **É a mesma
ideia da tabela de frequências**, com barras no lugar de números.

--

Por isso "78% das empresas pagam mais aos homens" (a maioria das barras está do lado roxo) é mais
defensável que uma média isolada: usa a distribuição inteira, não um único número resumo.
...
## Primeira rodada — Questão 1

Volte às duas Provocações de abertura (eixo cortado; pizza de 6 fatias). Para cada uma, proponha
o gráfico alternativo que resolveria o problema e explique, formalmente, por que resolve.

---

## Primeira rodada — Questão 2

Veja a notícia abaixo, sobre consumo médio de refrigerante por domicílio/ano:

```{r fig-refri, echo=FALSE, out.width="50%"}
knitr::include_graphics("images/refri_estatisticas.png")
```

--

```{r fig-refri-grafico, echo=FALSE, fig.height=2.4, fig.width=5}
refri <- tibble(
...
## Primeira rodada — Questão 3

Refaça o histograma de faturamento com `bins = 5` e depois com `bins = 60`. Em cada extremo, que
conclusão errada um leitor apressado poderia tirar?
